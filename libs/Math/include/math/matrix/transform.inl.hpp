...
{
	auto [m11, m12, m13] = rotation.row<1>();
	auto [m21, m22, m23] = rotation.row<2>();
	auto [m31, m32, m33] = rotation.row<3>();
...
constexpr auto TransformMatrix::construct(const Quat& rotation, const Vec3& origin) -> Super
{
	...
	auto [m11, m12, m13] = rot_matrix.row<1>();
	auto [m21, m22, m23] = rot_matrix.row<2>();
	auto [m31, m32, m33] = rot_matrix.row<3>();
